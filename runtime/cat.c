...
/*
 * Parse elem, the elem could be single number/rand_range or '(' ')' group
 * 1) A single number elem, it's just a simple digit. e.g. 9
 * 2) A single rand_range elem, two digits with a '-' between. e.g. 2-6
 * 3) A group elem, combines multiple 1) or 2) with '( )'. e.g (0,2-4,6)
 *    Within group elem, '-' used for a rand_range separator;
...
parse_set(const char *input, rte_cpuset_t *cpusetp)
{
	...
	/* process single number or single rand_range of number */
	if (*str != '(') {
		errno = 0;
 ...
 * cpus could be a single digit/rand_range or a group.
 * '(' and ')' are necessary if it's a group.
 *
